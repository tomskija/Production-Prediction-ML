    ...
    try:
        ...
        with mlflow.start_run(run_name=f"pipeline_{inputData['name']}") as run:
            ...
            if inputData['run_bnn']:
                (p, _, _, _, _), (_, bnn_results, _, _, _, _) = await asyncio.gather(
                ...
                # RF vs BNN comparison plot — requires both pipelines complete
                plot_rf_bnn_comparison(rf_p=p, bnn_results=bnn_results, path_db=path_db, sampling_method=best_method)
            else:
                p, _, _, _, _ = await run_rf_pipeline(
