...
class MCMCSampler:
    ...
    Uses torch.distributions.Normal for log likelihood and log prior computation.
    """
    ########################################################################################
    ...
    def log_likelihood(self, x=None, y=None, w=None, tau_sq=None):
        """
        Gaussian log likelihood using torch.distributions.Normal.
        p(y | w, tau) = Normal(pred, sqrt(tau_sq))
        """
        pred       = self.bnn.predict(x_np=x)
        pred_t     = torch.tensor(pred.ravel(),    dtype=torch.float32)
        y_t        = torch.tensor(y.ravel(),       dtype=torch.float32)
        dist       = Normal(loc=pred_t, scale=torch.tensor(tau_sq ** 0.5, dtype=torch.float32))
        log_lik    = dist.log_prob(y_t).sum().item()
        return log_lik, pred

    ########################################################################################
    def log_prior(self, w=None, tau_sq=None):
        """
        Log prior using torch.distributions.Normal for weights and Jeffreys prior for tau.
        p(w) = Normal(0, sqrt(sigma_sq))
        p(tau) ~ 1/tau  (Jeffreys)
        """
        w_t        = torch.tensor(w, dtype=torch.float32)
        w_dist     = Normal(loc=torch.zeros_like(w_t), scale=torch.tensor(self.sigma_sq ** 0.5, dtype=torch.float32))
        w_prior    = w_dist.log_prob(w_t).sum().item()
...
def plot_bnn_results_pt(results={}, y_train=[], y_test=[], path_db='', sampling_method=''):
    ...
    ########################################################################################
    print(f"\nBNN (PT) Results — {sampling_method} (post burn-in samples {burnin_idx}+):")
